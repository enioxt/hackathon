...
export function nomeCamera(indice: number): string { return `cam-sint-${String(indice).padStart(3, "0")}`; }

/** Curva de trânsito por hora do dia (0-23) — pico de manhã e à tarde, madrugada quase vazia. Valores em "fator" 0..1. */
export function fatorHora(hora: number): number {
...
export function gerarLeitura(fonte_id: string, camera_id: string, ts: Date, semente: number): Record<string, unknown> {
