...
// FONTE_ID (prefeitura-demo) tem so_sintetico:true em fontes.json — só aceita origem "sintetico" (ver teste dedicado abaixo).
const leituraBase = () => ({
  fonte_id: FONTE_ID, camera_id: "cam-teste-01", ts: "2026-09-19T20:00:00Z",
  janela_s: 60, contagens: { automovel: 10, moto: 3 }, origem: "sintetico" as const,
...
test('fonte de demonstração (so_sintetico) recusa origem "medido" — dado fictício não pode se passar por medição real', async () => {
  const r = await post("/api/leituras", { ...leituraBase(), origem: "medido" });
  expect(r.status).toBe(422);
  const j = await r.json();
  expect(j.erro).toMatch(/sintetico/i);
  const estado = await (await get("/api/estado")).json();
  expect(estado.cameras.length).toBe(0); // nada foi gravado
});
