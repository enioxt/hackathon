# Regras de negócio e Critérios de aceite do MVP — FILTER (HISTÓRICO)
...
|---|---|---|---|
| RN-01 | A câmera conta automóvel, moto, ônibus, caminhão, bicicleta e pedestre — nunca rosto, nunca placa, nunca guarda a imagem. | Receber e processar imagem já é tratamento; a promessa defensável é **minimização + não identificação + não retenção**, não "não capturar". Sem isso não existe diferença para vigilância. | `cco/validar.ts` (recusa antes de gravar) + `cco/contrato/leitura.schema.json` |
