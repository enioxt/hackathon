...
- **Reais, listados mas NÃO consertados (destino não é óbvio):**
  - `ABRA-ESTE.html` e `README.md` linkam `docs/COMECE-AQUI.html` — arquivo existiu e foi **apagado no commit `5383787`** (verificado com `git log --diff-filter=D`); não há substituto óbvio no repositório atual. Decisão de recriar ou remover o link fica para o dono.
  ...
  - `painel/painel-claro.html` (rota dinâmica `/claro`, só existe em `painel/`) tem 2 links relativos `solucoes.html` e `o-que-patos-ja-faz.html` — esses dois arquivos só existem em `demo/`, não em `painel/`, e não têm rota dinâmica em `servidor.ts` (diferente dos outros itens do mesmo menu, que usam `/rota` + `data-estatico`). Quebrado nos dois modos (servido e estático). Não mexi: não sei se a correção é copiar os dois arquivos para `painel/` ou trocar o link — decisão de arquitetura do dono.
- **Depois do conserto:** 0 links quebrados na raiz e em `demo/` (excluindo os três casos acima, listados e não consertados por falta de destino óbvio ou por exigir decisão de arquitetura); `docs/` mantém 1 link quebrado (`COMECE-AQUI.html`, listado); `painel/` mantém 3 (2 de imagem em `claro/`+`layout-rafael/`, 1 de menu em `painel-claro.html`).
