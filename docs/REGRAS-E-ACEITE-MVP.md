...
> **Atualização de 21/09/2026:** este arquivo preserva o snapshot da banca. Para a evolução pós-hackathon, os gates canônicos estão em `ARQUITETURA-OBSERVABILIDADE-AUDITAVEL.md`. Já existe uma primeira conferência pessoa × máquina em vídeo de Patos, mas ela é pequena e não autoriza promessa de acurácia. O motor atual usa Ultralytics YOLO sob AGPL-3.0; produção comercial exige decisão explícita de licença/backend.
...
|---|---|---|---|
| RN-01 | O fluxo padrão processa temporariamente os quadros para contar classes; não executa OCR de placa nem reconhecimento facial e não persiste frames por padrão. | Receber/processar imagem é tratamento; a promessa defensável é **minimização + não identificação + não retenção**, não “não capturar”. | `motor/leitor-video/leitor.py` + `motor/entrada-de-dados/validar.ts` |
| RN-02 | Os papéis LGPD devem ser definidos no arranjo contratual real; a hipótese de desenho é prefeitura como controladora da finalidade e fornecedor atuando sob instruções no escopo contratado. | O papel jurídico não nasce do nome do software; depende de quem determina finalidade e meios no caso concreto. | `juridico/` + revisão antes do piloto |
...
| RN-06 | Erro de contagem é medido contra contagem manual e publicado, nunca prometido de antemão. | Uma amostra inicial já existe, mas validação exige volume, classes, ângulos e horários suficientes. | 🟡 primeira amostra em `demo/patos-real/relatorio.html`; ampliar antes do piloto |
...
| RN-08 | Os valores atuais são **hipóteses de precificação**, não tabela comercial fechada: diagnóstico e piloto são os formatos de entrada; recorrência deve ser recalculada por volume real de vídeo/processamento, suporte e custo de licença do backend. | O próprio estudo mostra que preço por câmera isolado não fecha em alguns cenários; economia unitária deve ser validada no piloto. | `docs/pesquisa/precificacao-decisao.md` + `docs/LICENCIAMENTO-MOTOR-VISAO.md` |
...
|---|---|---|---|---|
| CA-01 | Vídeo entra → contagem sai | Abrir uma câmera gravada e mostrar a contagem por tipo aparecendo | ✅ pipeline funciona · 🟡 acurácia em validação | Leitor em `motor/leitor-video/` (7 testes); existe primeira comparação pessoa × máquina em `demo/patos-real/relatorio.html`, ainda pequena. |
