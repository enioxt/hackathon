...
async function roteador(req: Request, server?: { requestIP(req: Request): { address: string } | null }): Promise<Response> {
  ...
  if (["/camadas","/operador","/insights","/ajuda"].includes(u.pathname)) { try { return new Response(readFileSync(RAIZ + u.pathname + ".html", "utf8"), { headers: { "content-type": "text/html; charset=utf-8" } }); } catch { return new Response("página em construção", { status: 503 }); } }
