...
| `painel/` (38 no topo, 91 no total) | O aplicativo do gestor (site servido por `servidor.ts`, casco único `demo/app.html` é a versão publicada dele) | Quem roda o servidor local | vivo — importa parte de `motor/entrada-de-dados/` mas também guarda cópias próprias dos mesmos arquivos |
...
- **Depois do conserto:** 0 links quebrados na raiz e em `demo/` (excluindo os dois casos acima, listados e não consertados por falta de destino óbvio); `docs/` mantém 1 link quebrado (`COMECE-AQUI.html`, listado).
